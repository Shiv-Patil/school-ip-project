...
class MDTabsScrollView(ScrollView):
    ...
    def goto(self, scroll_x: Union[float, None], scroll_y: Union[float, None]) -> None:
...
class MDTabsBar(MDCard):
    ...
    def get_rect_instruction(self):
        canvas_instructions = self.layout.canvas.before.get_group("Indicator_line")
        ...
        if len(self.children) >= 2:
            try:
                ...
                self.bind(
                    ...
                    text_color_active=widget.tab_label.setter("text_color_active"),
                    text_color_normal=widget.tab_label.setter("text_color_normal"),
                )

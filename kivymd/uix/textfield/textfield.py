...
class Validator:
    ...
    def is_date_valid(self, text: str) -> bool:
        ...
        if re.match(f"^({largs[fmt[0]]})/({largs[fmt[1]]})/({largs[fmt[2]]})$", text):
            ...
            try:
                datetime = date(int(largs["yyyy"]), int(largs["mm"]), int(largs["dd"]))
            except ValueError:
                return True
...
class MDTextField(
    ...
    def set_default_colors(
        ...
        for attr_name in self._attr_names_to_updated.keys():
            self._set_color(attr_name, self._attr_names_to_updated[attr_name], updated)
    ...
    def set_active_underline_color(self, color: list) -> None:
        ...
        Animation(_line_color_focus=color, duration=0.2, t="out_quad").start(self)
    ...
    def set_helper_text_color(self, color: list) -> None:
        ...
        Animation(_helper_text_color=color, duration=0.2, t="out_quad").start(self)

    def set_max_length_text_color(self, color: list) -> None:
        """Animates the color of the max length text."""

        Animation(_max_length_text_color=color, duration=0.2, t="out_quad").start(self)

    def set_icon_right_color(self, color: list) -> None:
        """Animates the color of the icon right."""

        Animation(_icon_right_color=color, duration=0.2, t="out_quad").start(self)

    def set_icon_left_color(self, color: list) -> None:
        """Animates the color of the icon left."""

        Animation(_icon_left_color=color, duration=0.2, t="out_quad").start(self)
    ...
    def set_hint_text_font_size(self, font_size: float) -> None:
        ...
        if self.mode != "round":
            Animation(_hint_text_font_size=font_size, duration=0.2, t="out_quad").start(
                self
            )
    ...
    def set_max_text_length(self) -> None:
        ...
        if self.max_text_length:
            self._max_length_label.text = f"{len(self.text)}/{self.max_text_length}"
    ...
    def set_text(self, instance_text_field, text: str) -> None:
        ...
        if (self.text and self.max_length_text_color) or self._get_has_error():
            self.error = True
        if self.text and self.max_length_text_color and not self._get_has_error():
            self.error = False
    ...
    def on_focus(self, instance_text_field, focus: bool) -> None:
        ...
        if focus:
            ...
            if self.helper_text_mode in ("on_focus", "persistent") and self.helper_text:
                Clock.schedule_once(
                    lambda x: self.set_helper_text_color(self.helper_text_color_focus)
                )
            if self.mode == "fill":
                Clock.schedule_once(
            ...
            if self.max_text_length:
                Clock.schedule_once(
                    lambda x: self.set_max_length_text_color(self.max_length_text_color)
                )
            if self.icon_right:
                Clock.schedule_once(
                    lambda x: self.set_icon_right_color(self.icon_right_color_focus)
                )
            if self.icon_left:
                Clock.schedule_once(
                    lambda x: self.set_icon_left_color(self.icon_left_color_focus)
                )
            ...
            if self.helper_text_mode == "persistent" and self.helper_text:
                Clock.schedule_once(
                    lambda x: self.set_helper_text_color(self.helper_text_color_normal)
                )
            ...
            if self.icon_right:
                Clock.schedule_once(
                    lambda x: self.set_icon_right_color(self.icon_right_color_normal)
                )
            if self.icon_left:
                Clock.schedule_once(
                    lambda x: self.set_icon_left_color(self.icon_left_color_normal)
                )
            if self.hint_text:
                Clock.schedule_once(
                    lambda x: self.set_hint_text_color(focus, self.error)
            ...
            Clock.schedule_once(lambda x: self.set_max_length_text_color([0, 0, 0, 0]))
            ...
            if self.error:
                ...
                Clock.schedule_once(
                    lambda x: self.set_static_underline_color(self.line_color_normal)
                )
    ...
    def on_error(self, instance_text_field, error: bool) -> None:
        ...
        if error:
            ...
            if self.helper_text_mode in ("on_focus", "on_error"):
                Clock.schedule_once(lambda x: self.set_helper_text_color([0, 0, 0, 0]))
            elif self.helper_text_mode == "persistent":
                Clock.schedule_once(
                    lambda x: self.set_helper_text_color(self.helper_text_color_normal)
                )

...
class CommonElevationBehavior(Widget):
    ...
    def set_shader_string(self, *args) -> None:
        ...
        self.context["shadow_color"] = list(map(float, self.shadow_color))[:-1] + [
            float(self.opacity)
        ]
    ...
    def on_elevation(self, instance, value) -> None:
        def on_elevation(*args):
            if hasattr(self, "context"):
                ...
                self.hide_elevation(True if (value <= 0 or self.disabled) else False)
    ...
    def on_size(self, *args) -> None:
        ...
        self.rect.size = (
            width
            + (self._elevation * self.shadow_softness / self.shadow_softness_size),
            height
            + (self._elevation * self.shadow_softness / self.shadow_softness_size),
        )

...
with open(os.path.join(uix_path, "swiper", "swiper.kv"), encoding="utf-8") as kv_file:
...
class _ItemsBox(AnchorLayout):
    ...
    def _set_size(self, *args):
        ...
        self.size = [
            window_size[0] - self._root.items_spacing * self._root.width_mult * 2,
            self._root.height,
        ]
...
class MDSwiperItem(MDBoxLayout):
    ...
    def _selected_size(self):
        size = [
            Window.size[0] - self._root.items_spacing * self._root.width_mult * 2,
            self._root.height,
        ]
        anim = Animation(
    ...
    def _dismiss_size(self):
        size = [
            Window.size[0] - self._root.items_spacing * (1 + self._root.width_mult) * 2,
...
class MDSwiper(MDScrollView):
    ...
    def get_current_item(self):
        ...
        return list(reversed(self.ids.anchor_scroll.children))[self._selected].children[
            0
        ]
    ...
    def _reset_size(self, *args):
        ...
        if self.get_current_index() == 0:
            ...
            view_x = (
                child.x - self.items_spacing * self.width_mult - self.items_spacing * 2
            )
        else:

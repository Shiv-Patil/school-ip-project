...
class BaseDialogPicker(
    ...
    def on_input_field_text_color(self, instance, value: str | list | tuple) -> None:
        """For supported of current API."""
...
class MDDatePicker(BaseDialogPicker):
    ...
    def transformation_from_dialog_input_date(
        ...
        Animation(_shift_dialog_height=dp(0), _scale_calendar_layout=1, d=0.15).start(
            self
        )
        Animation(
            opacity=1,
    ...
    def compare_date_range(self) -> None:
        ...
        if self.max_date <= self.min_date:
            raise DatePickerTypeDateError(
                "`max_date` value cannot be less than or equal " "to 'min_date' value"
            )
    ...
    def update_calendar(self, year, month) -> None:
        ...
        for widget, widget_date in zip_longest(self._calendar_list, dates):
            ...
            widget.is_range_start = (
                visible and self.min_date is not None and widget_date == self.min_date
            )
            widget.is_range_end = (
                visible and self.max_date is not None and widget_date == self.max_date
            )

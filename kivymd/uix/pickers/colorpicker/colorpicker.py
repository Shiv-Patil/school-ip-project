...
class GradientTab(ThemableBehavior, MDBoxLayout):
    ...
    def create_gradient_texture(
        ...
        if not self.color_picker.default_color:
            ...
            r, g, b = [
                int(value * 255) for value in self.color_picker.default_color[:-1]
            ]
        ...
        for i in range(gradient_widget_width):
            ...
            draw.line((i, 0, i, gradient_widget_width), fill=(int(r), int(g), int(b)))
    ...
    def create_canvas_with_gradient_texture(self, interval: Union[int, float]) -> None:
    ...
    def updated_canvas(self, widget, touch, color=None) -> None:
        ...
        if not color:
            ...
            self.color_picker.dispatch("on_select_color", [x / 255.0 for x in color])
        else:
...
class MDColorPicker(BaseDialog):
    ...
    def update_color_slider_item_bottom_navigation(self, color: list) -> None:
        ...
        if "select_alpha_channel_widget" in self._current_tab.ids:
            self._current_tab.ids.select_alpha_channel_widget.ids.slider.color = color
    ...
    def get_rgb(self, color: list) -> list:
        ...
        return [
            int(value * 255) for value in (color[:-1] if len(color) == 4 else color)
        ]
    ...
    def on_type_color(
        ...
        if self._rgb:
            ...
            if type_color == "RGB":
                ...
                color = (
                    f"RGB({', '.join([str(value) for value in self.selected_color])})"
                )
    ...
    def on_switch_tabs(
        ...
        if name_tab == "bottom navigation gradient":
            ...
            instance_slider_tab = bottom_navigation_instance.ids.tab_manager.get_screen(
                "tune"
            ).children[0]
            ...
            try:
                ...
                color_list_tabs.background_color = [x / 255.0 for x in color][:-1] + [1]

...
class AmPmSelector(ThemableBehavior, MDBoxLayout):
    ...
    def _upadte_color(self, *args):
        bg_color = (
            self.owner.accent_color if self.owner.accent_color else self.bg_color_active
        )
        if self.selected == "am":
            self._am_bg_color = bg_color
            self._pm_bg_color = (
                self.owner.primary_color if self.owner.accent_color else self.bg_color
            )
...
class TimeInputTextField(MDTextField):
    ...
    def on_focus(self, *args) -> None:
        super().on_focus(*args)
        if self.text.strip():
            if not self.focus and int(self.text) == 0 and self.num_type == "hour":
                self.text = "12"
        else:
...
class MDTimePicker(BaseDialogPicker):
    ...
    def _update_pos_size(self, orientation, anim=False):
        ...
        time_input_pos = (
            ...
            else ([dp(24), dp(178)] if orientation == "landscape" else [dp(24), dp(96)])
        )
        if anim:
        ...
        am_pm_pos = (
            ...
            if orientation == "portrait"
                ...
                [dp(24), dp(126)] if orientation == "landscape" else [dp(252), dp(96)]
            )
        )
        am_pm_size = (
            [dp(52), dp(80)]
            if orientation == "portrait"
            else ([dp(216), dp(40)] if orientation == "landscape" else [dp(48), dp(70)])
        )
        if anim:
            Animation(
                pos=am_pm_pos,
        ...
        time_picker_size = (
            ...
            if orientation == "portrait"
                ...
                [dp(584), dp(368)] if orientation == "landscape" else [dp(324), dp(218)]
            )
        )
        if anim:
            Animation(
    ...
    def _switch_input(self):
        self._update_pos_size(
            self.theme_cls.device_orientation if self._state == "input" else "input",
            anim=True,
        )

...
class TableRecycleGridLayout(FocusBehavior, LayoutSelectionBehavior, RecycleGridLayout):
...
class CellHeader(MDTooltip, BoxLayout):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.sort_action:
            ...
            if self.is_sorted:
                ib.icon = "arrow-down" if self.sorted_order == "ASC" else "arrow-up"
                ib.size = [dp(24), dp(24)]
                ib.opacity = 1
            else:
...
class TableData(RecycleView):
        ...
        if self._row_data_parts:
            ...
            for j, x in enumerate(data):
                if x[0] == x[1]:
                    ...
                    if (isinstance(x[0], tuple) or isinstance(x[0], list)) and len(
                        x[0]
                    ) == 3:
                        r_data["icon"] = x[0][0]
                    ...
                    elif (isinstance(x[0], tuple) or isinstance(x[0], list)) and len(
                        x[0]
                    ) == 2:
                        r_data["icon"] = x[0][0]
    ...
    def set_text_from_of(self, direction: str) -> None:
        ...
        if self.pagination:
            ...
            self.pagination.ids.label_rows_per_page.text = (
                f"{self._current_value}-{self._to_value} " f"of {len(self.row_data)}"
            )
    ...
    def on_pagination(self, instance_table_date, instance_table_pagination) -> None:
        if self._to_value < len(self.row_data):
            self.pagination.ids.button_forward.disabled = False
...
class CellRow(
    ...
    def notify_checkbox_click(self, instance_check: MDCheckbox, active: bool) -> None:
    ...
    def select_check(self, instance_table_data: MDDataTable, active: bool) -> None:
        """Called upon activation/deactivation of the checkbox."""

        if active:
            if self.table._rows_number not in self.table.current_selection_check:
                self.table.current_selection_check[self.table._rows_number] = []
            if (
                self.index
                not in self.table.current_selection_check[self.table._rows_number]
            ):
                self.table.current_selection_check[self.table._rows_number].append(
                    self.index
                )
        else:
            if self.table._rows_number in self.table.current_selection_check:
                if (
                    self.index
                    in self.table.current_selection_check[self.table._rows_number]
                    and not active
                ):
                    self.table.current_selection_check[self.table._rows_number].remove(
                        self.index
                    )
    ...
    def on_table(self, instance_cell_row, instance_table_data: TableData) -> None:

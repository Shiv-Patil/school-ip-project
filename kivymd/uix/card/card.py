...
with open(os.path.join(uix_path, "card", "card.kv"), encoding="utf-8") as kv_file:
...
class MDCard(
    ...
    def __init__(self, *args, **kwargs):
        ...
        Clock.schedule_once(lambda x: self.on_ripple_behavior(0, self.ripple_behavior))
    ...
    def update_md_bg_color(self, instance_card, theme_style: str) -> None:
        if self.md_bg_color in self._bg_color_map:
            self.md_bg_color = get_color_from_hex(colors[theme_style]["CardsDialogs"])
...
class MDCardSwipe(MDRelativeLayout):
    ...
    def open_card(self) -> None:
        ...
        anim = Animation(x=swipe_x, t=self.opening_transition, d=self.opening_time)

...
with open(os.path.join(uix_path, "chip", "chip.kv"), encoding="utf-8") as kv_file:
    ...
    def do_animation_check(self, md_bg_color: list, scale_value: int) -> None:
        ...
        if not self.icon_left:
            if scale_value:
                ...
                Animation(size=(0, 0), t="out_sine", d=0.1).start(self.ids.relative_box)
...
ScreenManager:
    ...
    class MyScreen(MDScreen):
        def removes_marks_all_chips(self, selected_instance_chip=None, multiple=True):
            if multiple:
                for instance_chip in self.ids.chip_box.children:
                    if instance_chip.active:
                        instance_chip.active = False
